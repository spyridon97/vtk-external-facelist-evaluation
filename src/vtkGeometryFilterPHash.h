/*=========================================================================
  ...
  Module:    vtkGeometryFilterPHash.h
...
=========================================================================*/
// https://gitlab.kitware.com/vtk/vtk/-/blob/b2bdf26f6b45b38b08f105374306664bd0a1843c/Filters/Geometry/vtkGeometryFilter.h
/**
 * @class   vtkGeometryFilterPHash
 * @brief   extract boundary geometry from dataset (or convert data to polygonal type)
 *
 * vtkGeometryFilterPHash is a general-purpose filter to extract dataset boundary
 ...
 *
 * Another important feature of vtkGeometryFilterPHash is that it preserves
 ...
 * of vtkGeometryFilterPHash.
 *
 * @warning
 * While vtkGeometryFilterPHash and vtkDataSetSurfaceFilter perform similar operations,
 * there are important differences as follows:
 * 1. vtkGeometryFilterPHash can preserve (using RemoveGhostInterfaces) topological connectivity.
 *    vtkDataSetSurfaceFilter produces output primitives which may be disconnected from one another.
 * 2. vtkGeometryFilterPHash can generate output based on cell ids, point ids, and/or
 *    extent (bounding box) clipping. vtkDataSetSurfaceFilter strictly extracts
 *    the boundary surface of a dataset.
 * 3. vtkGeometryFilterPHash is much faster than vtkDataSetSurfaceFilter, because it's
 *    multi-threaded. As a result, vtkDataSetSurfaceFilter will delegate the processing
 *    of linear unstructured grids to vtkGeometryFilterPHash.
 * 4. vtkGeometryFilterPHash can (currently) only handle linear cells. The filter
...
#ifndef vtkGeometryFilterPHash_h
#define vtkGeometryFilterPHash_h
...
struct vtkGeometryFilterPHashHelper;

// Used to coordinate delegation to vtkDataSetSurfaceFilter
struct VTKFILTERSGEOMETRY_EXPORT vtkGeometryFilterPHashHelper
{
  enum CellType
  {
    VERTS = 0,
  ...
  static vtkGeometryFilterPHashHelper* CharacterizeUnstructuredGrid(vtkUnstructuredGridBase*);
  static void CopyFilterParams(vtkGeometryFilterPHash* gf, vtkDataSetSurfaceFilter* dssf);
  static void CopyFilterParams(vtkDataSetSurfaceFilter* dssf, vtkGeometryFilterPHash* gf);
  bool HasOnlyVerts()
  {
...
class VTKFILTERSGEOMETRY_EXPORT vtkGeometryFilterPHash : public vtkPolyDataAlgorithm
{
public:
  ///@{
  /**
   ...
   */
  static vtkGeometryFilterPHash* New();
  vtkTypeMacro(vtkGeometryFilterPHash, vtkPolyDataAlgorithm);
  ...
  ///@{
  /**
   * If PieceInvariant is true, vtkGeometryFilterPHash requests
   ...
   * this face should not be also extracted via the vtkGeometryFilterPHash. (This
  ...
  int UnstructuredGridExecute(
    vtkDataSet* input, vtkPolyData* output, vtkGeometryFilterPHashHelper* info, vtkPolyData* exc);
...
protected:
  vtkGeometryFilterPHash();
  ~vtkGeometryFilterPHash() override;
...
private:
  vtkGeometryFilterPHash(const vtkGeometryFilterPHash&) = delete;
  void operator=(const vtkGeometryFilterPHash&) = delete;
};
