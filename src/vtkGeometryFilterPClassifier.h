/*=========================================================================
  ...
  Module:    vtkGeometryFilterPClassifier.h
...
=========================================================================*/
// https://gitlab.kitware.com/vtk/vtk/-/blob/c44fd2ba0d86779d65f5eda56a1aaceff2954807/Filters/Geometry/vtkGeometryFilter.h
/**
 * @class   vtkGeometryFilterPClassifier
 * @brief   extract boundary geometry from dataset (or convert data to polygonal type)
 *
 * vtkGeometryFilterPClassifier is a general-purpose filter to extract dataset boundary
 ...
 *
 * Another important feature of vtkGeometryFilterPClassifier is that it preserves
 ...
 * vtkGeometryFilterPClassifier can be slow. Consequently the filter has an optional
 ...
 * execution of vtkGeometryFilterPClassifier.
 *
 ...
 * of vtkGeometryFilterPClassifier.
 *
 * @warning
 * While vtkGeometryFilterPClassifier and vtkDataSetSurfaceFilter perform similar operations,
 * there are important differences as follows:
 * 1. vtkGeometryFilterPClassifier preserves topological connectivity. vtkDataSetSurfaceFilter
 *    produces output primitives which may be disconnected from one another.
 * 2. vtkGeometryFilterPClassifier can generate output based on cell ids, point ids, and/or
 *    extent (bounding box) clipping. vtkDataSetSurfaceFilter strictly extracts
 *    the boundary surface of a dataset.
 * 3. vtkGeometryFilterPClassifier is much faster than vtkDataSetSurfaceFilter, especially
 *    for vtkUnstructuredGrids. As a result, vtkDataSetSurfaceFilter will
 *    delegate the processing of linear unstructured grids to vtkGeometryFilterPClassifier.
 * 4. vtkGeometryFilterPClassifier can (currently) only handle linear cells. The filter
...
#ifndef vtkGeometryFilterPClassifier_h
#define vtkGeometryFilterPClassifier_h
...
struct vtkGeometryFilterPClassifierHelper;
struct vtkExcludedFaces;

// Used to coordinate delegation to vtkDataSetSurfaceFilter
struct VTKFILTERSGEOMETRY_EXPORT vtkGeometryFilterPClassifierHelper
{
  unsigned char IsLinear;
  static vtkGeometryFilterPClassifierHelper* CharacterizeUnstructuredGrid(vtkUnstructuredGrid*);
  static void CopyFilterParams(vtkGeometryFilterPClassifier* gf, vtkDataSetSurfaceFilter* dssf);
  static void CopyFilterParams(vtkDataSetSurfaceFilter* dssf, vtkGeometryFilterPClassifier* gf);
};

class VTKFILTERSGEOMETRY_EXPORT vtkGeometryFilterPClassifier : public vtkPolyDataAlgorithm
{
public:
  ///@{
  /**
   ...
   */
  static vtkGeometryFilterPClassifier* New();
  vtkTypeMacro(vtkGeometryFilterPClassifier, vtkPolyDataAlgorithm);
   ...
   * this face should not be also extracted via the vtkGeometryFilterPClassifier. (This
  ...
  int UnstructuredGridExecute(
    vtkDataSet* input, vtkPolyData* output, vtkGeometryFilterPClassifierHelper* info, vtkExcludedFaces* exc);
...
protected:
  vtkGeometryFilterPClassifier();
  ~vtkGeometryFilterPClassifier() override;
...
private:
  vtkGeometryFilterPClassifier(const vtkGeometryFilterPClassifier&) = delete;
  void operator=(const vtkGeometryFilterPClassifier&) = delete;
};
